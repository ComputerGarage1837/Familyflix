...
function Get-FFDeploymentConfig {
    ...
    [pscustomobject]@{
        ...
        PreparationPluginRoot = 'C:\ProgramData\Jellyfin\Server\plugins\Family Flix Watchlists_1.0.0.3'
        PreparationPluginSha256 = '06820040505AA7B868AEE563B23048FCEE634BD1402AC7687ED39C4D4DB18240'
        PreparationProcessId = 7656
        ...
        OriginalIndexSha256 = '247B2344F7EA604BF63F01389D3A8259E6339CB0C3EEB33C472F9B9603B02399'
...
function Get-FFServerState {
    param([switch]$PreparationBaseline)
    $ffConfig = Get-FFDeploymentConfig
    ...
    $ffPluginRoot = if ($PreparationBaseline) { $ffConfig.PreparationPluginRoot } else { $ffConfig.PluginRoot }
    $ffExpectedPluginHash = if ($PreparationBaseline) { $ffConfig.PreparationPluginSha256 } else { $ffConfig.PluginSha256 }
    $ffPlugin = Join-FFRelativePath -Root $ffPluginRoot -Relative $ffConfig.PluginName -MustExist -Kind File
    $ffProcesses = @(Get-Process -Name jellyfin -ErrorAction Stop)
    if ($ffProcesses.Count -ne 1 -or ($PreparationBaseline -and $ffProcesses[0].Id -ne $ffConfig.PreparationProcessId)) {
    ...
    # executable version, authenticated server API, and plugin hash remain the guard.
    ...
    if ($ffPluginHash -ne $ffExpectedPluginHash) { throw 'The Family Watchlist plugin hash changed.' }
    ...
    return [pscustomobject]@{
        ...
        state = $(if ($PreparationBaseline) { 'pre-activation-package-baseline' } else { 'post-activation-deployment-target' })
        protectedFiles = @($ffProtected)
    }
}

function Assert-FFPreparationServerRecord {
    param($State)
    $ffConfig = Get-FFDeploymentConfig
    if ($null -eq $State -or $State.processId -ne $ffConfig.PreparationProcessId -or
        $State.stockVersion -ne $ffConfig.StockVersion -or
        $State.pluginSha256 -ne $ffConfig.PreparationPluginSha256 -or
        $State.state -ne 'pre-activation-package-baseline') {
        throw 'The package does not record the pinned pre-activation server/plugin baseline.'
    }
    # Reuse the strict timestamp parser and full process comparator against itself.
    Assert-FFSameServer -Before $State -After $State
}
...
function Get-FFStagedHtml {
    ...
    $ffConfig = Get-FFDeploymentConfig
    if ($BuiltHtml -notmatch '<meta\s+name="familyflix-web-build"\s+content="0\.19\.10-family\.28"\s*/?>' -or
...
function Get-FFVerifiedPackage {
    ...
    if ((Split-Path -Parent $ffPackage) -ne $ffConfig.DeploymentRoot -or
        (Split-Path -Leaf $ffPackage) -notmatch '^family28-package-\d{8}-\d{6}-\d{3}-[a-f0-9]{8}$') {
        throw 'Choose a timestamped family28 package directly inside the reviewed deployment directory.'
    }
    ...
    Assert-FFPreparationServerRecord $ffManifest.serverBefore
