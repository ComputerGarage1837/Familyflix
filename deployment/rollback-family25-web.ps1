...
$ffHelper = Get-Item -LiteralPath (Join-Path $PSScriptRoot 'family25-web-common.ps1') -Force
...
try {
    ...
    if ((Split-Path -Parent $ffValidatedBackup) -ne $ffConfig.BackupRoot -or
        (Split-Path -Leaf $ffValidatedBackup) -notmatch '^web-family25-\d{8}-\d{6}-\d{3}-[a-f0-9]{8}$') {
        throw 'Choose the exact timestamped family25 backup directory recorded by the installer.'
    }
    ...
    $ffPending = Join-FFRelativePath -Root $ffConfig.WebRoot -Relative ('familyflix-rollback-family25-' + $ffStamp + '.pending') -Kind File
    ...
    $ffRetiredIndex = Join-FFRelativePath -Root $ffValidatedBackup -Relative ('index.family25-before-rollback-' + $ffStamp + '.html') -Kind File
    ...
    Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('rollback-family25-' + $ffStamp + '.json')) -Text $ffText
    ...
    if (-not $VerifyOnly) {
        ...
        try {
            ...
            Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('rollback-family25-' + $ffStamp + '-failure.json')) -Text $ffText
