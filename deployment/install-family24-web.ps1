...
try {
    ...
    $ffBackupPath = Join-FFRelativePath -Root $ffConfig.BackupRoot -Relative ('web-family24-' + $ffStamp) -Kind Directory
    ...
    $ffPendingIndex = Join-FFRelativePath -Root $ffConfig.WebRoot -Relative ('familyflix-entry-family24-' + $ffStamp + '.pending') -Kind File
    ...
    Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family24-' + $ffStamp + '.json')) -Text $ffResultText
    ...
    if (-not $VerifyOnly) {
        ...
        try {
            Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family24-' + $ffStamp + '-failure.json')) -Text $ffFailureText
