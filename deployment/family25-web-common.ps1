...
function Get-FFDeploymentConfig {
    ...
    [pscustomobject]@{
        ...
        PluginRoot = 'C:\ProgramData\Jellyfin\Server\plugins\Family Flix Watchlists_1.0.0.3'
        PluginName = 'Jellyfin.Plugin.FamilyWatchlist.dll'
        PluginSha256 = '06820040505AA7B868AEE563B23048FCEE634BD1402AC7687ED39C4D4DB18240'
        BackupRoot = 'C:\ProgramData\Jellyfin\Server\data\FamilyFlixWebBackups'
        StockVersion = '10.11.5'
        ProcessId = 7656
        OriginalIndexSha256 = '7FE965DFEAC7CC3926CDF59E3B9C198688D32B6CC31BA677106C5925927F9943'
...
function Get-FFStagedHtml {
    ...
    $ffConfig = Get-FFDeploymentConfig
    if ($BuiltHtml -notmatch '<meta\s+name="familyflix-web-build"\s+content="0\.19\.10-family\.25"\s*/?>' -or
...
function Get-FFVerifiedPackage {
    ...
    if ((Split-Path -Parent $ffPackage) -ne $ffConfig.DeploymentRoot -or
        (Split-Path -Leaf $ffPackage) -notmatch '^family25-package-\d{8}-\d{6}-\d{3}-[a-f0-9]{8}$') {
        throw 'Choose a timestamped family25 package directly inside the reviewed deployment directory.'
    }
