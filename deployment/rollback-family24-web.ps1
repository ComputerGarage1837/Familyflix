...
$ffHelper = Get-Item -LiteralPath (Join-Path $PSScriptRoot 'family24-web-common.ps1') -Force
...
try {
    ...
    if ((Split-Path -Parent $ffValidatedBackup) -ne $ffConfig.BackupRoot -or
        (Split-Path -Leaf $ffValidatedBackup) -notmatch '^web-family24-\d{8}-\d{6}-\d{3}-[a-f0-9]{8}$') {
        throw 'Choose the exact timestamped family24 backup directory recorded by the installer.'
    }
    ...
    $ffPending = Join-FFRelativePath -Root $ffConfig.WebRoot -Relative ('familyflix-rollback-family24-' + $ffStamp + '.pending') -Kind File
    ...
    $ffRetiredIndex = Join-FFRelativePath -Root $ffValidatedBackup -Relative ('index.family24-before-rollback-' + $ffStamp + '.html') -Kind File
    ...
    $ffResult = [ordered]@{
        ...
        restoredIndexSha256 = (Get-FFSha256 $ffLiveIndex); retiredFamily24Index = $ffRetiredIndex
    ...
    Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('rollback-family24-' + $ffStamp + '.json')) -Text $ffText
    ...
    if (-not $VerifyOnly) {
        ...
        try {
            ...
            Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('rollback-family24-' + $ffStamp + '-failure.json')) -Text $ffText
