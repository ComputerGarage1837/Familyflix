...
try {
    ...
    Assert-FFSameServer -Before $ffManifest.serverBefore -After $ffServerBefore
    ...
    $ffBackupPath = Join-FFRelativePath -Root $ffConfig.BackupRoot -Relative ('web-family25-' + $ffStamp) -Kind Directory
    ...
    $ffPendingIndex = Join-FFRelativePath -Root $ffConfig.WebRoot -Relative ('familyflix-entry-family25-' + $ffStamp + '.pending') -Kind File
    ...
    Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family25-' + $ffStamp + '.json')) -Text $ffResultText
    ...
    if (-not $VerifyOnly) {
        ...
        try {
            Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family25-' + $ffStamp + '-failure.json')) -Text $ffFailureText
