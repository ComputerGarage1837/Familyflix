...
try {
    ...
    $ffBackupPath = Join-FFRelativePath -Root $ffConfig.BackupRoot -Relative ('web-family29-' + $ffStamp) -Kind Directory
    ...
    $ffPendingIndex = Join-FFRelativePath -Root $ffConfig.WebRoot -Relative ('familyflix-entry-family29-' + $ffStamp + '.pending') -Kind File
    ...
    Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family29-' + $ffStamp + '.json')) -Text $ffResultText
    ...
    if (-not $VerifyOnly) {
        ...
        try {
            Write-FFNewText -Root $ffConfig.DeploymentRoot -Path (Join-Path $ffConfig.DeploymentRoot ('install-family29-' + $ffStamp + '-failure.json')) -Text $ffFailureText
