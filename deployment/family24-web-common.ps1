...
function Get-FFDeploymentConfig {
    ...
    [pscustomobject]@{
        ...
        PluginRoot = 'C:\ProgramData\Jellyfin\Server\plugins\Family Flix Watchlists_1.0.0.2'
        PluginName = 'Jellyfin.Plugin.FamilyWatchlist.dll'
        PluginSha256 = 'B62F374B8F67B295D5AE2123EAFC1DE701B28E26CB83AF2959222434C4B40989'
        BackupRoot = 'C:\ProgramData\Jellyfin\Server\data\FamilyFlixWebBackups'
        StockVersion = '10.11.5'
        ProcessId = 17992
        OriginalIndexSha256 = '8FF711029C7441C47EBCBD8D06A713E1BF998FDECCE5D057728BEB38E2338C41'
...
function Get-FFServerState {
    ...
    if ($ffProcesses.Count -ne 1 -or $ffProcesses[0].Id -ne $ffConfig.ProcessId -or
        -not $ffProcesses[0].Path.Equals($ffExe, [StringComparison]::OrdinalIgnoreCase)) {
    ...
    return [pscustomobject]@{
        ...
        processPath = $ffProcesses[0].Path
...
function Get-FFStagedHtml {
    ...
    $ffConfig = Get-FFDeploymentConfig
    if ($BuiltHtml -notmatch '<meta\s+name="familyflix-web-build"\s+content="0\.19\.10-family\.24"\s*/?>' -or
...
function Get-FFVerifiedPackage {
    ...
    if ((Split-Path -Parent $ffPackage) -ne $ffConfig.DeploymentRoot -or
        (Split-Path -Leaf $ffPackage) -notmatch '^family24-package-\d{8}-\d{6}-\d{3}-[a-f0-9]{8}$') {
        throw 'Choose a timestamped family24 package directly inside the reviewed deployment directory.'
    }
