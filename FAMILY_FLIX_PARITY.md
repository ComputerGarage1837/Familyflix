...
| Kids Mode | Per-profile local settings for enablement, spoiler hiding, bedtime, episode limit and parent PIN coded; profile access requires PIN when Kids Mode is on; playback checks bedtime | Verify PIN/profile switching, hidden art/text and bedtime on Windows; add automatic-next behavior before episode-limit control can take effect; test session sleep timer |
| Backdrops, themes and detail actions | Native details, focus-driven backdrops with thumbnail fallback and 60-second random idle rotation, show/season/episode navigation and cast (episode guest stars, season cast, show fallback), watched/unwatched action, compact single-row actions and 15 per-user Android palette choices coded | Port remaining Android detail actions and remote focus rules; verify cast, every theme contrast and image fallback on real media |
